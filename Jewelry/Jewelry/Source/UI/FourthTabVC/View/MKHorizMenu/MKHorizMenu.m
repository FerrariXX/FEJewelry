...
-(void) buttonTapped:(id) sender
{
    ...
    for(int i = 0; i < self.itemCount; i++)
    {
        ...
        if(i + kButtonBaseTag == button.tag){
            thisButton.selected = YES;
            NSLog(@"%@",NSStringFromCGPoint(thisButton.frame.origin));
            if ((thisButton.frame.origin.x  > 240 || self.contentOffset.x  > 10) && (i < self.itemCount -2)) {
                [self setContentOffset:CGPointMake(thisButton.frame.origin.x - kLeftOffset, 0) animated:YES];
            }
